...
public class ExcelDeliberationExporter {
	...
	private void writeDataLines() {
		...
		for (String[] datum : data) {
			...
			int columnCount = 0;
			for (String s : datum) {
				if(columnCount == datum.length - 2) createCell(row, columnCount++, Double.parseDouble(s), style);
				else createCell(row, columnCount++, s, style);
				
			}
			// row.getCell(columnCount-1).setCellFormula("RANG(AI"+(columnCount+1)+";$AI$6:$AI$"+(data.length+6)+";0)");
		}
	}
